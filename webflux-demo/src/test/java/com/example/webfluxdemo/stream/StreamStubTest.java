...
public class StreamStubTest {
    ...
    @Test
    public void sumValuesInListThreadUnsafe() throws Exception{

        List<Integer> myNumbersToSum = Arrays.asList(1,3,5,7,6,2,4,1);

        int auxVarToSum = 0;

        for (Integer item : myNumbersToSum){
            auxVarToSum += item;
        }
        System.out.println("The sum is: " + auxVarToSum);
    }

    @Test
    public void sumValuesInListThreadSafe(){

        List<Integer> myNumbersToSum = Arrays.asList(1,3,5,7,6,2,4,1);

        int sumWithStream = myNumbersToSum.stream()
                .reduce(0, Integer::sum);

        int sumWithStreamOlder = myNumbersToSum.stream()
                .reduce(0, (prev, next) -> prev + next);

        int mulWithStream = myNumbersToSum.stream()
                        .reduce(1, (prev, next) -> prev * next);

        System.out.println("Mul result: " + mulWithStream);

        System.out.println("The sum using stream: " + sumWithStream);
    }
